...
function CustomTypeEditor() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success(t('customTypes.createdToast'))
      navigate({ to: '/dashboard/custom-types' })
    },
    onError: (error: Error) => {
      toast.error(t('customTypes.createFailed', { message: error.message }))
    }
  })
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['actionItems'] })
      toast.success(t('customTypes.savedToast'))
    },
    onError: (error: Error) => {
      toast.error(t('customTypes.saveFailed', { message: error.message }))
    }
  })

  const handleSave = () => {
    if (!name.trim()) {
      toast.error(t('customTypes.validationName'))
      return
    }
    const keys = fields.map((f) => f.key.trim()).filter((k) => k)
    if (new Set(keys).size !== keys.length) {
      toast.error(t('customTypes.validationUniqueKeys'))
      return
    }
    if (fields.length === 0 || fields.every((f) => !f.key.trim())) {
      toast.error(t('customTypes.validationAtLeastOne'))
      return
    }
        ...
        <div className="max-w-6xl mx-auto px-8 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            >
              ...
              <span className="text-sm">{t('customTypes.breadcrumbTypes')}</span>
            </Button>
            <span className="text-muted-foreground/30">/</span>
            <span className="text-sm text-muted-foreground truncate max-w-[200px]">
              {isNew ? t('customTypes.newType') : name || t('customTypes.untitled')}
            </span>
          </div>
          <div className="flex items-center gap-2">
            ...
            >
              ...
              {t('customTypes.preview')}
            </Button>
            <Button size="sm" className="h-8" onClick={handleSave} disabled={isSaving}>
              {isSaving ? t('customTypes.saving') : isNew ? t('customTypes.create') : t('customTypes.save')}
            </Button>
          </div>
        </div>
      </div>
          ...
          <div className="space-y-10">
            <div className="space-y-5 flex items-start">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 space-y-2 min-w-0">
                  <input
                    ...
                    placeholder={t('customTypes.untitledTypePlaceholder')}
                  ...
                  <textarea
                    ...
                    placeholder={t('customTypes.descriptionPlaceholder')}
                    rows={1}
              ...
              <div className="flex flex-col gap-3 items-end">
                <div className="flex items-center gap-3">
                  <span className="text-xs text-muted-foreground/60">{t('customTypes.statusLabel')}</span>
                ...
                <div className="flex items-center gap-3">
                  <span className="text-xs text-muted-foreground/60">{t('customTypes.categoryLabel')}</span>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="text-sm font-semibold text-foreground">{t('customTypes.properties')}</h2>
                  <p className="text-xs text-muted-foreground/60 mt-0.5">
                    {t('customTypes.propertiesHint')}
                  </p>
                </div>
                <Button
                  variant="ghost"
              ...
              {fields.length === 0 ? (
                ...
                >
                  ...
                  {t('customTypes.addFirstProperty')}
                </button>
              ) : (
                  ...
                  <div className="flex items-center gap-2 px-2 py-1.5">
                    ...
                    <div className="flex-1 grid grid-cols-[1fr_1fr_120px_120px] gap-2">
                      <span className="text-[10px] font-medium text-muted-foreground/50 uppercase tracking-wider">
                        {t('customTypes.columnKey')}
                      </span>
                      <span className="text-[10px] font-medium text-muted-foreground/50 uppercase tracking-wider">
                        {t('customTypes.columnLabel')}
                      </span>
                      <span className="text-[10px] font-medium text-muted-foreground/50 uppercase tracking-wider">
                        {t('customTypes.columnType')}
                      </span>
                      <span className="text-[10px] font-medium text-muted-foreground/50 uppercase tracking-wider">
                        {t('customTypes.columnFormat')}
                      </span>
                    </div>
