    ...
    <div>
      <Sheet open={openFlowSheet} onOpenChange={setOpenFlowSheet}>
        <SheetContent className="sm:max-w-xl">
          <SheetHeader>
            <SheetTitle>
              {t('flows.addConnectorTo')}{' '}
              <span className="text-primary">{selectedNode?.data.class_name}</span>
            </SheetTitle>
            <SheetDescription>{t('flows.chooseEnricher')}</SheetDescription>
          </SheetHeader>
          <div className="p-4 grow overflow-auto border-t">
            ...
            {isLoading && (
              <div className="flex items-center justify-center py-8">
                ...
                <span className="ml-2">{t('flows.loadingEnrichers')}</span>
              </div>
            )}
            ...
            {error && (
              <Alert variant="destructive" className="mb-4">
                ...
                <AlertDescription>{t('flows.enrichersLoadError')}</AlertDescription>
              </Alert>
            )}
              ...
              <div className="flex flex-col gap-2">
                {filteredEnrichers.length === 0 ? (
                  <div className="text-center py-8 text-muted-foreground">
                    {searchTerm.trim()
                      ? t('flows.noEnrichersMatch')
                      : t('flows.noEnrichersForNode')}
                  </div>
                ) : (
            ...
            <div className="space-y-1 truncate text-left">
              ...
              <div className="mt-2 flex items-center gap-2 text-xs">
                <div className="flex items-center gap-1">
                  <span className="text-muted-foreground">{t('flows.takes')}</span>
                ...
                <div className="flex items-center gap-1">
                  <span className="text-muted-foreground">{t('flows.returns')}</span>
