...
export function TypePreview({
  ...
  return (
    <div className="space-y-6">
      {/* Entity card preview */}
      <div>
        <p className="text-xs font-medium text-muted-foreground mb-3">{t('customTypes.entityCard')}</p>
          ...
          <div className="flex items-start gap-3.5">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2">
                <h4 className="font-semibold text-sm truncate">{name || t('customTypes.untitledType')}</h4>
              </div>
            ...
            <div className="mt-4 pt-3 border-t border-border/40 space-y-2">
              ...
              {validFields.length > 4 && (
                <p className="text-[10px] text-muted-foreground/50">
                  {t('customTypes.moreFields', { count: validFields.length - 4 })}
                </p>
              )}
            </div>
          )}
      ...
      {/* Graph node preview */}
      <div>
        <p className="text-xs font-medium text-muted-foreground mb-3">{t('customTypes.graphNode')}</p>
        <div className="flex items-center gap-3 p-4 border border-border/60 rounded-xl bg-card max-w-[200px]">
          ...
          <div className="min-w-0">
            <p className="text-xs font-medium truncate">{name || t('customTypes.untitled')}</p>
            <p className="text-[10px] text-muted-foreground">
              {t('customTypes.fieldCount', { count: validFields.length })}
            </p>
          </div>
        </div>
      </div>
      ...
      {/* Schema preview */}
      <div>
        <p className="text-xs font-medium text-muted-foreground mb-3">{t('customTypes.jsonSchema')}</p>
