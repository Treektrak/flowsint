...
export default function NewSketch({ children }: NewSketchProps) {
  ...
  const createSketchMutation = useMutation({
    ...
    onSuccess: (result) => {
      if (result.id) {
        toast.success(t('dialogs.sketchCreated'))
        router.navigate({
        ...
        toast.error(result.error || t('dialogs.sketchCreateFailed'))
      }
    },
    onError: (error) => {
  ...
  async function onSubmit(data: FormValues) {
    if (!investigationId) {
      toast.error(t('dialogs.sketchRequiresInvestigation'))
      return
    }
    ...
    <form onSubmit={handleSubmit(onSubmit)}>
      <div className="grid gap-4 py-4">
        <div className="grid gap-2">
          <Label htmlFor="title">{t('dialogs.sketchName')}</Label>
          <Input
            id="title"
            {...register('title', { required: t('dialogs.validationTitleRequired') })}
            placeholder={t('dialogs.sketchNamePlaceholder')}
        ...
        <div className="grid gap-2">
          ...
          <Input
            ...
            placeholder={t('dialogs.sketchDescriptionPlaceholder')}
          />
        </div>
      </div>
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>{t('dialogs.newSketch')}</DialogTitle>
          <DialogDescription>{t('dialogs.newSketchDescription')}</DialogDescription>
        </DialogHeader>
        {formContent}
      </DialogContent>
    </Dialog>
